  ...
  gardenShare = pkgs.writeShellApplication {
    ...
    text = ''
      usage() {
        ...
        echo ""
        echo "  note-path   e.g. slips/1721566840-walking-skeleton.md"
        echo "  slug        short code for the URL, e.g. walking-skeleton-li"
        echo ""
        echo "  utm_source  WHERE traffic comes from: linkedin, github, mastodon, discord, twitter"
        echo "  utm_medium  HOW they get there:       social, referral, chat, email"
        echo "  utm_campaign WHY you're sharing:      content-share, launch, weekly-notes"
        exit 1
      }
...
in {
  age.secrets."shlink/key".file = "${self}/data/secrets/shlink/key.age";

{...}: {
  flake.modules.homeManager.healthchecks = {
    config,
    lib,
    pkgs,
    ...
    ...
    with lib; let
      ...
      healthCheckScript = pkgs.writeShellScript "healthcheck" ''
        ...
        if [ -n "$failed_hosts" ]; then
          ...
          ${pkgs.libnotify}/bin/notify-send -u critical "Health Check Failed" "$message"
        ''}
          exit 1
        fi
      '';
    in {
      options.maintenance.healthchecks = {
        ...
        addresses = mkOption {
          ...
          default = [];
          example = ["firesprout.home.deraedt.dev" "deraedt.dev"];
          description = "List of IP addresses or domains to monitor";
        };

        notifications.enable = mkEnableOption "desktop notifications for failed health checks";
      ...
      config = mkIf cfg.enable {
        systemd.user.services.healthcheck = {
          Unit.Description = "Health check monitoring service";
          Service = {
            Type = "oneshot";
        ...
        systemd.user.timers.healthcheck = {
          Unit.Description = "Timer for health check monitoring";
          Timer = {
            OnBootSec = "1min";

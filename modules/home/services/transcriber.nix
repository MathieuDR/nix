{...}: {
  flake.modules.homeManager.transcriber = {
    pkgs,
    config,
    ...
  }: let
    ...
    transcribeScript = pkgs.writeShellApplication {
      ...
      runtimeInputs = with pkgs; [coreutils gnused ffmpeg jq whisper-ctranslate2 libnotify];
      text = ''
        INPUT_FILE="$1"
    ...
    watchScript = pkgs.writeShellApplication {
      name = "watch-memos";
      runtimeInputs = with pkgs; [coreutils inotify-tools libnotify];
      text = ''
  ...
  in {
    ...
    services.syncthing.settings.folders."recordings" = {
      enable = true;
      path = memosDir;
      id = "ncp43-9drkl";
      label = "Recordings";
